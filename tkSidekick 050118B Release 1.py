...
root.wm_title("V:050118B (c) 2018 John A. Oakey" + " "*150 + "tkSideKick")
...
#  DATA, INFORMATION, CONSTANTS, TEXT, TUPLES, DICTIONARIES, ETC.

# IGNORE ALL THE TEXT STUFF DOWN TO ABOUT LINE 240 - IT WILL JUST CONFUSE THE WEAK MINDED
...
cb1=Checkbutton(top1, text="Choice #1", variable=var1, onvalue=1, command=cb1callback)
cb2=Checkbutton(top1, text="Choice #2", variable=var1, onvalue=2, command=cb1callback)
cb1.pack()
cb2.pack()''',
...
def cb1callback():
    ...
    cb2.config(bg="white")
    eval("cb" + str(var1.get())).config(bg="red")
cb1=Checkbutton(f1, text="Choice #1", variable=var1, onvalue=1, command=cb1callback)
cb2=Checkbutton(f1, text="Choice #2", variable=var1, onvalue=2, command=cb1callback)
cb1.pack()
cb2.pack()
f1.pack()''',
...
l1.pack()''',
'Label' : '''# Label - a text message or a label - semi-permanent
...
'Message' : '''#Message - a multiline variatnt of Label
m_txt = "Red Leicester? \nIm afraid we are fresh out of Red Leicester sir.\nTilsit? \nNever \
...
displaytopmenu.add_command(label="(*Instantiate Note)", foreground='grey40', command=lambda
    cmd='Tipsnote': econtrol(cmd))
...
displayhelpmenu.add_command(label="Inspect Object Search", command=lambda cmd="obhlp": econtrol(cmd))
displayhelpmenu.add_command(label="Pydoc Search Docs", command=lambda cmd="py_lib_doc": econtrol(cmd))
...
def cpyrte():
    cpyrte_text = "tksidekick copyright 2018 by John A. Oakey\n Version: 050118a\nAll commercial " \
                ...
                "www.wikipython.com is a non-commerical hobby enterprise.\n\nPlease send " \
                  "comments to: oakey.john@yahoo.com\n\n" \
                "More information about tksidekick and a download url for GitHub may (or may not) " \
                "be available on www.wikipython.com\n\nMany thanks for:\n\nAdditional coding and concepts:\n" \
                "Victor Domingos (Portugal, https://no-title.victordomingos.com)\n\n" \
                  "Beta Testing:\n" \
                  "Chadrack Besongo (scadhsong@gmail.com)\n" \
                  "Victor Domingos\nJason Orr"
...
def obj_explore():
    ...
    headertitle(lftop, "Inspect an Object")  # reestablish panel heads
    headertitle(lfbottom, "Information for " + objstr)
    obj = simpledialog.askstring("Inspect Report for an Object", "Enter a possible\
 object term (like, 'Button', 'ord', or 'print')", parent=t1)
    t1.delete(1.0, END)
    t1.insert(END, "Inspect sometimes takes a few seconds.\n")
...
def py_lib_doc():
    ...
    headertitle(lfbottom, " Information for " + objstr)
    obj = simpledialog.askstring("Pydoc Help Report for an Object", "Enter a possible\
                                 object term (like, 'Button', 'ord', or 'print')", parent=t1)
    objstr = obj  # make a copy
    if obj is None or type(obj) is 'NoneType' or obj == "":   # handle cancel or no response
        obj = "No input"
    ...
    try:
        ...
        if objstr.lower() in badwidget:  # check for bad widget capitalization is eval fails
            ...
            reset("2")
            t2.insert(END, "One moment - trying to research " + objstr + " . ")
            helpout = helpout + obj + " " + str(e) + " - can not evaluate as an object." + rtn  # use original value
            t2.insert(END, helpout)
            return
    finally:
...
def pddisplay():
    ...
    scrollbar = Scrollbar(t1, bd=7, cursor="cross", background="alice blue")
